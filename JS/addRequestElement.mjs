export const addRequestElement = (requestText, requestStatus) => {
    ...
    if (requestStatus === 200){
        const jsonDataFromPhp = JSON.parse(requestText)
        div.classList.add(`${jsonDataFromPhp.isIn ? 'greenStyle' : 'redStyle'}`)
        ...
        td22.insertAdjacentText('beforeend', `(${jsonDataFromPhp.x},${jsonDataFromPhp.y},${jsonDataFromPhp.r})`)
        td32.insertAdjacentText('beforeend', `${jsonDataFromPhp.isIn ? 'hit' : 'no hit'}`)
        td42.insertAdjacentText('beforeend', `${jsonDataFromPhp.date}`)
        td52.insertAdjacentText('beforeend', `${jsonDataFromPhp.time}`)
